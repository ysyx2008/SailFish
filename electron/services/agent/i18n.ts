...
const translations = {
  'zh-CN': {
    ...
    'agent.plan_incomplete': '计划中还有 {count} 个步骤未完成：\n{steps}\n\n继续之前请先评估：\n- 如果任务目标已达成，调用 clear_plan 归档计划并给出总结\n- 如果只是部分步骤不再需要，用 update_plan 标记为 skipped 后继续\n- 如果需要继续执行，执行下一步并更新状态',
    'agent.user_supplement_with_plan': '[系统提示：用户补充了新信息。请根据这些信息继续执行。执行后请评估：如果任务目标已提前达成，调用 clear_plan 归档；如果部分步骤不再需要，标记为 skipped。]',
...
[系统/任务的当前状态]`,
    ...
    'im.tool_unsupported_image_format': '不支持的图片格式: {ext}，请使用 send_file_to_chat 发送非图片文件',
  ...
  'en-US': {
    ...
    'agent.plan_incomplete': '{count} steps in the plan are incomplete:\n{steps}\n\nBefore continuing, please evaluate:\n- If the task goal is achieved, call clear_plan to archive and provide summary\n- If only some steps are no longer needed, mark them as skipped with update_plan and continue\n- If execution should continue, proceed with the next step and update status',
    'agent.user_supplement_with_plan': '[System: User provided new info. Please continue based on this info. After execution, evaluate: if the task goal is achieved early, call clear_plan to archive; if some steps are no longer needed, mark them as skipped.]',
...
[Current state of system/tasks]`,
    ...
    'im.tool_unsupported_image_format': 'Unsupported image format: {ext}. Use send_file_to_chat for non-image files.',
