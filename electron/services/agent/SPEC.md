...
> - **成功标准**：
>   1. 任务栏上把它列成和严格、宽松、自由并列的第四档，栏上只写现在会怎样。选「替我审批」等于宽松，并且本来要问你的那一下先替你看。默认关。底下不是第四种执行规则。
>   2. 只接本来就要弹批准框的那一下。硬墙照旧直接拦；宽松里本来不问的照旧不问；你点过「本次允许」的照旧不问。
...
> - **明确不做**：不沿用上一次的结论（同类操作也逐条评）；不在后台预先打分、按分数免评；不截你的原话，装不下就直接问你；不换更便宜的模型、不调低思考强度（和干活那个用同一个模型）；不把被拦的操作改走联络去问你（人就在这场任务前面）；不在远程机器上借你的终端窗格跑核查命令。
>
