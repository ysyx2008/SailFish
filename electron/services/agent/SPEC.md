...
> - **成功标准**：
>   1. 输入框里把它列成和严格、宽松、自由并列的第四档，写现在会怎样，放在发送前。选「替我审批」等于宽松，并且本来要问你的那一下先替你看。默认关，界面上标成推荐。首页和对话是同一档，记住你上次选的。底下不是第四种执行规则。不为了防认级漏判去评每一步：只读、没风险的不必再过一轮。
...
- **当次任务累计消耗**：默认不显示，设置里打开后出现在输入框模型选择右侧，与进度条的「窗口占用」分开。发请求时数字立刻跳起，生成过程中持续跳动，悬停看出入明细。只在本次对话过程中累计，不写入历史；新对话归零。
...
输入框把「替我审批」列成第四档（选它等于宽松，再加先替你看）。底下仍是上面三档；「替我审批」决定要批准的那一下先由谁看（见上文设计目标）。
