...
⚠️ 仅支持本地文件系统搜索，不支持 SSH 远程主机。`,
    ...
    } as ToolDefinitionWithMeta,
    {
      ...
      function: {
        name: 'remember_info',
        description: `保存有价值的信息到知识库，未来交互时自动提供。只记录**非显而易见的、对未来任务有帮助的**信息。

记录：用户偏好与习惯、项目背景与上下文、人物关系与联系方式、工作流程与惯例、环境配置与路径、问题排查经验、领域知识与决策依据。
不记录：系统提示中已有的信息、用户规则已写明的内容、操作系统通用常识、纯动态数据（瞬时 CPU/内存）、大段原始输出。

记录前自问：这条信息能帮未来的 Agent 更好完成任务吗？关键细节（名称、路径、版本等）必须准确。`,
        parameters: {
          type: 'object',
          properties: {
            info: {
