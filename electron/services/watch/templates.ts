...
将结果保存到共享状态：
SHARED_UPDATE: {"lastEmailSummary": "摘要内容", "lastEmailPriority": "紧急程度"}`,
      skills: ['email'],
      execution: { type: 'local' },
      output: { type: 'notification' },
      priority: 'normal'
...
将部署状态保存到工作流状态：
STATE_UPDATE: {"lastDeploy": "时间", "status": "成功/失败", "version": "版本号"}`,
      execution: { type: 'local' },
      output: { type: 'im' },
      priority: 'high'
    })
...
只有在发现异常时才生成完整报告。
将关键指标保存到状态：
STATE_UPDATE: {"cpuUsage": "XX%", "memUsage": "XX%", "diskUsage": "XX%", "checkedAt": "时间"}`,
