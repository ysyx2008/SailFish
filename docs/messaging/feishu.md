...
Follow these steps to create a Feishu app and connect it to SFTerminal. For an overview of messaging integration, see [Messaging Integration Guide](./README.md).
...
Your Feishu app is ready. Keep SFTerminal connected; optional: check **Auto-connect on startup**. See [Connect in SFTerminal](./README.md#connect-in-sfterminal) and [Usage](./README.md#usage) for full details.

### FAQ: "App Has No Active Long Connection"

This is expected. You must connect from SFTerminal first (Step 5) to establish a WebSocket long connection before the Feishu platform allows saving the long connection subscription configuration.
