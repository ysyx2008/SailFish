...
- 🧩 **技能系统**：按需加载扩展能力（如 Excel 处理），避免工具过载
...
### v8.7.0 (当前版本)
- 🧩 **技能系统（Skills）**：全新的可扩展技能架构
  - 技能是一组相关工具的集合，按需动态加载
  - 避免工具过多导致 AI 选择困难
  - 使用 `load_skill` 工具加载所需技能
- 📊 **Excel 处理技能**：首个内置技能，提供会话式 Excel 文件读写能力
  - `excel_open` - 打开 Excel 文件（支持创建新文件）
  - `excel_read` - 读取数据（支持指定 Sheet 和单元格范围）
  - `excel_modify` - 修改单元格、添加/删除 Sheet、支持公式
  - `excel_save` - 保存修改（自动创建备份）
  - `excel_close` - 关闭文件
  - 适用于数据分析、报表处理、批量数据修改等场景
